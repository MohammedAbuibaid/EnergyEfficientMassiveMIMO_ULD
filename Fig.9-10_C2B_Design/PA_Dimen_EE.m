function [EEopt,Ropt,Mopt] = PA_Dimen_EE(PLO,PLI,Kmax,Mmax,Pc,PmaxPAcriteria,PmaxPA,fxy,plott)
...
%   function [EE,Rc,Ptot,QoS] = EE_R_Ptot_PA(PLO,PLI,Kmax,K,Mc,Md,p,Pmax_PA,mimR,fxy)
         [EE(M,K),R(M,K),~,~] = EE_R_Ptot_PA(PLO,PLI,K,K,M,M*ones(1,18),p,PmaxPA,[],fxy);
    end
end

[EEopt,Mopt]  = max(EE);
[EEgOpt, KgOpt] = max(EEopt);
MgOpt = Mopt(KgOpt);
...
if plott ==true
    ...
    txt = {'Global Optimum:',['M = ' num2str(MgOpt) ', K = ' num2str(Kmax)],['EE = ' num2str(EEgOpt/1e6) ' Mbit/J']};
    ...
    figure
    subplot(3,1,1)
    plot(1:Kmax,EEopt/1e6)
    title(['Max EE can be obtained for K = 1:' num2str(Kmax) ', [Mbit/Joule]'])
    grid on
    
    subplot(3,1,2);
    plot(1:Kmax,Mopt)
    title(['Optimal M for K = 1:' num2str(Kmax)])
    grid on
    
    subplot(3,1,3);
    plot(1:Kmax,Ropt/1e6)
