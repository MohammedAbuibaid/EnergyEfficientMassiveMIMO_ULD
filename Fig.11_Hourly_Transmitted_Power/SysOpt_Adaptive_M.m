function [Mopt,AvgEEperBS, AvgURperBS, AvgPtotperBS ]=SysOpt_Adaptive_M(PLO_Network,PLI_Network,KgOpt,Mmin,MgOpt,Pc,PmaxPA,loading,lambdaS)

EE_per_BS =zeros(19,1);
UR_per_BS =zeros(19,1);
Ptot_per_BS =zeros(19,1);
    ...
    for BSj=1:19
        ...
        for K=1:KgOpt
            for M=Mmin:MgOpt;
                Mc = M; Md = Md_temp;
                p = Pc/M;
                [EE(M,K),UserRate(M,K),Ptot(M,K)] = EE_R_Ptot_PA(PLO_Network{BSj},PLI_Network{BSj},KgOpt,K,Mc,Md,p,PmaxPA,[]);
            end
        end
        [EEopt, Mopt]=max(EE);
        disp(['Mopt_perUS = ' num2str(Mopt)])
        Ropt = zeros(1,KgOpt);
        PWRopt = zeros(1,KgOpt);

        for K=1:KgOpt
            Ropt(K) =  UserRate(Mopt(K),K);
            PWRopt(K) = Ptot(Mopt(K),K);
        end
        Pi = zeros(1,KgOpt);
        for K=1:KgOpt
            [Pi(K),~] = MGmm_SD_Queue(K,KgOpt,loading*lambdaS,1,Ropt);
        end
